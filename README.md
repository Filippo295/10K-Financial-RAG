...
**At each query:**
1. FAISS semantic search and BM25 keyword search each retrieve 20 candidate chunks independently. Reciprocal Rank Fusion (RRF) merges the two ranked lists and selects the top 10.
...
- Hybrid retrieval (FAISS + BM25 + RRF) was chosen over just semantic search because embeddings capture meaning but struggle with exact numerical figures and specific financial terms; BM25 keyword matching solves this by finding chunks that contain the exact query terms
